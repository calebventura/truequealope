...
const productSchema = z
  .object({
    ...
    acceptedExchangeTypes: z
      .array(
        z.enum(
          ["money", "product", "service", "exchange_plus_cash", "giveaway"] as const
        )
      )
      .min(1, "Selecciona una opción de intercambio"),
  ...
  .superRefine((data, ctx) => {
    ...
    if (types.includes("exchange_plus_cash")) {
      ...
      if (!data.wantedProducts?.trim() && !data.wantedServices?.trim()) {
        ctx.addIssue({
          code: z.ZodIssueCode.custom,
          path: ["wantedProducts"],
          message: "Debes especificar qué artículo o servicio buscas recibir",
        });
      }
    }
    ...
    if (
      (types.includes("product") || types.includes("service")) &&
      !types.includes("exchange_plus_cash")
    ) {
...
export default function NewProductPage() {
  ...
  const { checking: checkingProfile } = useProfileCompletionGuard(
    user,
    authLoading
  );
  ...
  useEffect(() => {
    if (!authLoading && !user) {
      router.replace("/auth/login?next=/products/new");
    }
